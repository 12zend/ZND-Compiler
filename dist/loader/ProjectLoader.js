...
export class ProjectLoader {
    ...
    async extractFromBuffer(id, buffer) {
        ...
        const json = JSON.parse(projectJsonFile.content);
        const project = { id, json, assets: new Map() };
        for (const file of extracted.files) {
            if (file.name !== 'project.json' && file.content) {
                const key = file.name.replace(/^.*\//, '');
                project.assets.set(key, file.content);
            }
        }
        await this.cache.setCompiled(id, project);
        return project;
    }
    createAssetManifest(json) {
        const costumes = [];
        const sounds = [];
        let totalSize = 0;
    ...
    async loadAssets(project, manifest) {
        const loadCostume = async (asset) => {
    ...
    async preloadAssets(manifest, onProgress) {
        ...
        const loadCostumeImage = async (asset) => {
    ...
    extract(project) {
        return this.extractFromBuffer('temp', project);
    }
}
