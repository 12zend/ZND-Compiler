import type { 
  IRProgram, IRBlock, IRValue, IRSprite, CompiledProgram, CompiledScript,
  GeneratedCode, GPUOperation, StaticAnalysisResult
} from '../types/ir';
import type { IRVariable, IRList, IRScript } from '../types/ir';
import { BlockParser } from './BlockParser';

const GPU_PRIORITY: Record<string, 'high' | 'medium' | 'low'> = {
...
export class JSCodeGenerator {
  ...
  generateProgram(ir: IRProgram): CompiledProgram {
    ...
    const spriteInitCode = new Map<string, string>();
  ...
  generateScript(sprite: IRSprite, script: IRScript): CompiledScript {
    ...
    const gpuOps: GPUOperation[] = this.extractGPUOps(script.topBlock);
  ...
  private generateGlobalInit(ir: IRProgram): string {
    const lines: string[] = [
  ...
  private generateSpriteInit(sprite: IRSprite): string {
    const lines: string[] = [
      `// Sprite: ${sprite.name}`,
  ...
  private generateBlockCode(block: IRBlock, ctx: GenerationContext): string {
    switch (block.type) {
      case 'start':
  ...
  private generateStartBlock(block: IRBlock, ctx: GenerationContext): string {
    if (!block.next) return '';
    return this.generateBlockCode(block.next, ctx);
  }

  private generateWaitBlock(block: IRBlock, ctx: GenerationContext): string {
  ...
  private generateForeverBlock(block: IRBlock, ctx: GenerationContext): string {
    const currentIndent = this.indent();
    this.indentLevel++;
  ...
  private generateIfBlock(block: IRBlock, ctx: GenerationContext): string {
  ...
  private generateIfElseBlock(block: IRBlock, ctx: GenerationContext): string {
    const condition = this.evaluateCondition(block.inputs.CONDITION, ctx);
    const currentIndent = this.indent();
    this.indentLevel++;
  ...
  private generateBroadcastBlock(block: IRBlock, ctx: GenerationContext): string {
  ...
  private generateBroadcastWaitBlock(block: IRBlock, ctx: GenerationContext): string {
    const message = this.evaluateInput(block.inputs.BROADCAST_INPUT, ctx);
    const lines: string[] = [
  ...
  private generateSetVariableBlock(block: IRBlock, ctx: GenerationContext): string {
  ...
  private generateChangeVariableBlock(block: IRBlock, ctx: GenerationContext): string {
    const varName = this.getVariableRef(block.fields.VARIABLE?.name || '', ctx);
  ...
  private generateOperatorBlock(block: IRBlock, ctx: GenerationContext): string {
  ...
  private generateOperator(block: IRBlock): string {
  ...
  private mathFunction(op: string): string {
  ...
  private generateMotionBlock(block: IRBlock, ctx: GenerationContext): string {
  ...
  private generateLooksBlock(block: IRBlock, ctx: GenerationContext): string {
    const sprite = this.getSpriteVarName(ctx.sprite.id);
    let code = '';
  ...
  private generateExpressionBlock(block: IRBlock, ctx: GenerationContext): string {
    if (block.next) {
      return this.generateBlockCode(block.next, ctx);
    }
    return '';
  ...
  private generateCloneBlock(block: IRBlock, ctx: GenerationContext): string {
  ...
  private generateStopBlock(block: IRBlock, ctx: GenerationContext): string {
    const stopOption = block.fields.STOP_OPTION?.name || 'all';
    return `yield* stop('${stopOption}');`;
  }

  private generateGenericBlock(block: IRBlock, ctx: GenerationContext): string {
    if (block.next) {
      return this.generateBlockCode(block.next, ctx);
    }
    return '';
  ...
  private evaluateInput(input: IRValue | IRValue[] | undefined, ctx?: GenerationContext): string {
  ...
  private getOperand(input: IRValue | IRValue[] | undefined): string {
    return this.evaluateInput(input);
  }

  private evaluateCondition(input: IRValue | IRValue[] | undefined, ctx: GenerationContext): string {
    const value = this.evaluateInput(input, ctx);
    return `!!(${value})`;
  }

  private getVariableRef(name: string, ctx: GenerationContext): string {
    return `${this.getSpriteVarName(ctx.sprite.id)}.variables['${name}']`;
  }

  private getSpriteVarName(id: string): string {
  ...
  private detectYield(block: IRBlock): boolean {
  ...
  private detectYieldById(blockId: string): boolean {
    return false;
  }

  private detectScriptType(script: IRScript): 'sync' | 'async' | 'generator' {
  ...
  private collectGlobals(block: IRBlock): string[] {
    const globals = new Set<string>();
    const collect = (b: IRBlock) => {
      for (const input of Object.values(b.inputs)) {
        if (Array.isArray(input)) {
          for (const val of input) {
            if (val.type === 'variable') globals.add(val.value as string);
          }
        } else if (input.type === 'variable') {
          globals.add(input.value as string);
        }
      }
  ...
  private collectFunctions(block: IRBlock): string[] {
  ...
  private collectDependencies(script: IRScript): string[] {
    return [];
  }

  private extractGPUOps(block: IRBlock): GPUOperation[] {
    const ops: GPUOperation[] = [];
    const collect = (b: IRBlock) => {
